...
for i in range(1, 2):
    powerup = Powerup(img_powerup, randint(30, win_width - 30), -40, 80, 50, randint(1, 7))
...
while run:
    ...
    if not finish:
        ...
        if sprite.spritecollide(ship, powerups, True):
            life = life + 1
    ...
    # бонус: автоматичний перезапуск гри
        ...
        for i in range(1, 3):
            ...
            asteroids.add(asteroid)
        for i in range(1, 3):
            powerup = Powerup(img_ast, randint(30, win_width - 30), -40, 80, 50, randint(1, 7))
            powerups.add(powerup)
